...
struct SwipstoneView: View {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 0) {
                if viewModel.state.getSwipstoneResponse[selectedCardIndex].collect && !viewModel.state.getSwipstoneResponse[selectedCardIndex].achieve {
                    ...
                    ScrollViewReader { proxy in
                        HStack(spacing: 25) {
                            ...
                            ForEach(viewModel.state.getSwipstoneResponse, id: \.id) { data in
                                SwipstoneCardView(point: data.point, collect: data.collect, achive: data.achieve, viewModel: viewModel, cardIndex: Int(data.id))
                                    .onTapGesture {
                    ...
                    .overlay {
                        ScrollView(.horizontal) {
                            HStack(spacing: 20 * Constants.ControlWidth) {
                                ForEach(viewModel.state.getSwipstonePieceResponse, id: \.region) { data in
                                    Image(data.region)
                                        .resizable()
                                        .scaledToFit()
                ...
                Button(action: {
                    if viewModel.state.getSwipstoneResponse[selectedCardIndex].achieve == false {
                        ...
                        if viewModel.state.getSwipstoneResponse[selectedCardIndex].collect == true {
                            ...
                            let usedPieceInfo = viewModel.pieceSubtract(selectedCardIndex: selectedCardIndex)
                            
                            Task{
                                await viewModel.action(.subtractPiece(usedPieceInfo: usedPieceInfo))
                            }
                        } else {
                            print("조각이 부족합니다.")
                        }
        ...
        .navigationDestination(for: swipstoneType.self) { view in
            switch view {
            case .guide:
                SwipstoneGuideView()
            }
...
struct SwipstoneCardView: View {
    ...
    var body: some View {
        ZStack{
            if point == "5000"{
                Image(achive == true ? "swipstone_card_collect_5000" : "swipstone_card_5000")
                    .resizable()
                    .scaledToFit()
                        ...
                        ZStack{
                            VStack(spacing: 0){
                                ZStack{
                                    ...
                                    Image(viewModel.getUpdatedPieceImage(cardIndex: cardIndex, pieceName: "sejong"))
                                        .resizable()
                                        .scaledToFit()
                                    ...
                                    Image(viewModel.getUpdatedPieceImage(cardIndex: cardIndex, pieceName: "gyeongnam"))
                                        .resizable()
                                        .scaledToFit()
                                        .frame(width: 231.16 * Constants.ControlWidth, height: 231.16 * Constants.ControlHeight)
                ...
                Image(achive == true ? "swipstone_card_collect_10000" : "swipstone_card_10000")
                    .resizable()
                    .scaledToFit()
                    .frame(width: 242 * Constants.ControlWidth, height: 456 * Constants.ControlHeight)
                    .overlay {
                        ZStack{
                            VStack(spacing: 0){
                                ...
                                ZStack{
                                    Image(viewModel.getUpdatedPieceImage(cardIndex: cardIndex, pieceName: "ulsan"))
                                        .resizable()
                                        .scaledToFit()
                                        .frame(width: 231.16 * Constants.ControlWidth, height: 231.16 * Constants.ControlHeight)
                                        .offset(y: 10 * Constants.ControlHeight)
                                    ...
                                    Image(viewModel.getUpdatedPieceImage(cardIndex: cardIndex, pieceName: "chungcheongnam"))
                                    .resizable()
                                    .scaledToFit()
                                    .frame(width: 231.16 * Constants.ControlWidth, height: 231.16 * Constants.ControlHeight)
                                    .offset(y: 180 * Constants.ControlHeight)
                ...
                Image(achive == true ? "swipstone_card_collect_15000" : "swipstone_card_15000")
                    .resizable()
                    .scaledToFit()
                    .frame(width: 242 * Constants.ControlWidth, height: 456 * Constants.ControlHeight)
                    .overlay {
                        ZStack{
                            VStack(spacing: 0){
                                ZStack{
                                    ...
                                    Image(viewModel.getUpdatedPieceImage(cardIndex: cardIndex, pieceName: "gwanju"))
                                        .resizable()
                                        .scaledToFit()
                                        .frame(width: 231.16 * Constants.ControlWidth, height: 231.16 * Constants.ControlHeight)
                                        .offset(y: 10 * Constants.ControlHeight)
                                    ...
                                    Image(viewModel.getUpdatedPieceImage(cardIndex: cardIndex, pieceName: "jeollanam"))
                                        .resizable()
                                        .scaledToFit()
                                        .frame(width: 231.16 * Constants.ControlWidth, height: 231.16 * Constants.ControlHeight)
                                    ...
                                    Image(viewModel.getUpdatedPieceImage(cardIndex: cardIndex, pieceName: "gangwon"))
                                        .resizable()
                                        .scaledToFit()
                                        .frame(width: 231.16 * Constants.ControlWidth, height: 231.16 * Constants.ControlHeight)
                                        .clipped()
